...
// auto run
func init() {
	source.RegisterInit(initOrderDictDetail, &initDictDetail{})
}
...
func (i *initDictDetail) MigrateTable(ctx context.Context) (context.Context, error) {
	...
	return ctx, db.AutoMigrate(&sysModel.SysDicDetail{})
}
...
func (i *initDictDetail) TableCreated(ctx context.Context) bool {
	...
	return db.Migrator().HasTable(&sysModel.SysDicDetail{})
}

func (i initDictDetail) InitializerName() string {
	return sysModel.SysDicDetail{}.TableName()
}
...
func (i *initDictDetail) InitializeData(ctx context.Context) (context.Context, error) {
	...
	dicts, ok := ctx.Value(initDict{}.InitializerName()).([]sysModel.SysDic)
	if !ok {
		return ctx, errors.Wrap(source.ErrMissingDependentContext,
			fmt.Sprintf("未找到 %s 表初始化数据", sysModel.SysDic{}.TableName()))
	}
	True := true
	dicts[0].Details = []sysModel.SysDicDetail{
		{Label: "男", Value: 1, Status: &True, Sort: 1},
		{Label: "女", Value: 2, Status: &True, Sort: 2},
	}

	dicts[1].Details = []sysModel.SysDicDetail{
	...
	dicts[2].Details = []sysModel.SysDicDetail{
	...
	dicts[3].Details = []sysModel.SysDicDetail{
	...
	dicts[4].Details = []sysModel.SysDicDetail{
	...
	dicts[5].Details = []sysModel.SysDicDetail{
		{Label: "tinyint", Value: 1, Extend: "mysql", Status: &True},
		{Label: "bool", Value: 2, Extend: "pgsql", Status: &True},
	}
	for _, dict := range dicts {
		if err := db.Model(&dict).Association("Details").
			Replace(dict.Details); err != nil {
			return ctx, errors.Wrap(err, sysModel.SysDicDetail{}.TableName()+"表数据初始化失败!")
		}
	}
	return ctx, nil
}
...
func (i *initDictDetail) DataInserted(ctx context.Context) bool {
	...
	var dict sysModel.SysDic
	if err := db.Preload("Details").
		First(&dict, &sysModel.SysDic{Name: "数据库bool类型"}).Error; err != nil {
		return false
	}
	return len(dict.Details) > 0 && dict.Details[0].Label == "tinyint"
}
