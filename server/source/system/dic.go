...
// auto run
func init() {
	source.RegisterInit(initOrderDict, &initDict{})
}
...
func (i *initDict) MigrateTable(ctx context.Context) (context.Context, error) {
	...
	return ctx, db.AutoMigrate(&sysModel.SysDic{})
}
...
func (i *initDict) TableCreated(ctx context.Context) bool {
	...
	return db.Migrator().HasTable(&sysModel.SysDic{})
}

func (i initDict) InitializerName() string {
	return sysModel.SysDic{}.TableName()
}
...
func (i *initDict) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	if err = db.Create(&entities).Error; err != nil {
		return ctx, errors.Wrap(err, sysModel.SysDic{}.TableName()+"表数据初始化失败!")
	}
...
func (i *initDict) DataInserted(ctx context.Context) bool {
	...
	if errors.Is(db.Where("type = ?", "bool").First(&sysModel.SysDic{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
