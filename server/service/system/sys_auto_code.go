...
func (autoCodeService *AutoCodeService) CreatePlug(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[2], "tpl") < 0 {
			dirPath := filepath.Join(global.CONFIG.AutoCode.Root, global.CONFIG.AutoCode.Server, fmt.Sprintf(global.CONFIG.AutoCode.SPlug, plug.Snake+"/"+pathArr[2]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.CONFIG.AutoCode.Root, global.CONFIG.AutoCode.Server, fmt.Sprintf(global.CONFIG.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugPath):len(tpl)-4]))
...
func (autoCodeService *AutoCodeService) InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
	...
	if webIndex != -1 {
		err = installation(paths[webIndex], global.CONFIG.AutoCode.Server, global.CONFIG.AutoCode.Web)
		if err != nil {
	...
	if serverIndex != -1 {
		err = installation(paths[serverIndex], global.CONFIG.AutoCode.Server, global.CONFIG.AutoCode.Server)
	}
	return webIndex, serverIndex, err
}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.ToSlash(global.CONFIG.AutoCode.Root + formPath + "/" + path)
	var to = filepath.ToSlash(global.CONFIG.AutoCode.Root + toPath + "/plugin/")
...
func (autoCodeService *AutoCodeService) PubPlug(plugName string) (zipPath string, err error) {
	...
	webPath := filepath.Join(global.CONFIG.AutoCode.Root, global.CONFIG.AutoCode.Web, "plugin", plugName)
	serverPath := filepath.Join(global.CONFIG.AutoCode.Root, global.CONFIG.AutoCode.Server, "plugin", plugName)
	// 创建一个新的zip文件
	...
	return filepath.Join(global.CONFIG.AutoCode.Root, global.CONFIG.AutoCode.Server, fileName), nil
}
