    ...
    <el-drawer v-if="drawer" v-model="drawer" custom-class="auth-drawer" :with-header="false" size="40%" title="角色配置">
      <el-tabs :before-leave="autoEnter" type="border-card">
        ...
        <el-tab-pane label="资源权限">
          <Datas ref="datas" :authority="table_data" :row="activeRow" @changeRow="changeRow" />
        </el-tab-pane>
      </el-tabs>
    </el-drawer>
  </div>
...
import Menus from '@/view/admin/auth/components/menus.vue'
import Apis from '@/view/admin/auth/components/apis.vue'
import Datas from '@/view/admin/auth/components/datas.vue'
...
const AuthorityOption = ref([{ authorityId: 0, authorityName: '根角色' }])
...
const form = ref({ authorityId: 0, authorityName: '', parentId: 0 })
const rules = ref({
  authorityId: [
...
const deleteAuth = (row) => {
  ElMessageBox.confirm('此操作将永久删除该角色, 是否继续?', '提示', {
    ...
    .then(async () => {
      const res = await auth_delete({ authorityId: row.authorityId })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功!'
