        ...
        <el-tree ref="menuTree" :data="menuTreeData" :default-checked-keys="menuTreeIds" :props="menuDefaultProps"
          ...
          <template #default="{ node, data }">
            <span class="custom-tree-node">
              ...
              <span>
                <el-button type="primary" link :style="{ color: row.defaultRouter === data.name ? '#E6A23C' : '#85ce61' }"
                  :disabled="!node.checked" @click="() => setDefault(data)">
...
<script setup>
import { menu_all, menu_get_auth, menu_create_auth } from '@/api/menu'
import { auth_update } from '@/api/auth'
...
const setDefault = async (data) => {
  const res = await auth_update({ authorityId: props.row.authorityId, AuthorityName: props.row.authorityName, parentId: props.row.parentId, defaultRouter: data.name })
  if (res.code === 0) {
