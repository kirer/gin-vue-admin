...
import { system_get_info } from '@/api/system'
//
...
const on_reload = async () => { const { data } = await system_get_info(); state.value = data.server; }
on_reload()
timer.value = setInterval(() => on_reload(), 1000 * 10)
onUnmounted(() => { clearInterval(timer.value); timer.value = null})
</script>
<style>
.card_item {
  @apply h-80 text-xl;
